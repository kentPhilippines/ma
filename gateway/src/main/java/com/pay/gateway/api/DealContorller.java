...
public class DealContorller {
	...
	public void payH5Ali(Deal deal ,HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		/*
		 * String channel = ""; ChannelFee channelFee =
		 * channelServiceImpl.findChanenlFeeByChannelNo(accountFee.getAccountChannel(),
		 * accountFee.getChannelProduct()); channel = channelFee.getRouting();
		 * if(StrUtil.isBlank(channel)) { log.info(
		 * "------------------------------【15002:当前商户账户渠道路由未开通】------------------------------"
		 * ); response.getWriter().write("15002:当前商户账户渠道路由未开通"); return; }
		 */
